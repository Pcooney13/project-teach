		...
		<div class="mb-12 md:mb-0 border-b shadow-xl rounded-xl sticky top-[5rem]">
			...
			<div class="p-4 bg-white rounded-b-xl flex flex-col gap-4">
				...
				if (!$teams) {
					...
					$consultant_hover_text_color = 'hover:text-green';
					$group_hover_text_color = 'group-hover:text-green';
					$bg_color = 'bg-green';
				} else {
				...
				<?php
				$hospitals = get_the_terms($post->ID, 'hospitals');
				if ($hospitals) {
			...
			<div>
				...
				<div class="hidden sm:block bg-white rounded-xl shadow">
					<nav class="grid md:grid-cols-3 text-center p-2">
						<button x-on:click="section1 = true, section2 = false, section3=false" :class="section1 === true ? `text-white <?php echo $bg_color; ?> font-gotham-medium shadow-sm` : `text-slate-500 font-gotham <?php echo $consultant_hover_text_color; ?>`" class="p-2 rounded-lg <?php echo $text_color; ?> font-gotham-medium">
							About
						</button>
						<button x-on:click="section2 = true, section1 = false, section3=false" :class="section2 === true ? `text-white <?php echo $bg_color; ?> font-gotham-medium shadow-sm` : `text-slate-500 font-gotham <?php echo $consultant_hover_text_color; ?>`" class="p-2 rounded-lg <?php echo $text_color; ?> font-gotham-medium">
							Events
						</button>
						<button x-on:click="section3 = true, section1 = false, section2=false" :class="section3 === true ? `text-white <?php echo $bg_color; ?> font-gotham-medium shadow-sm` : `text-slate-500 font-gotham <?php echo $consultant_hover_text_color; ?>`" class="p-2 rounded-lg <?php echo $text_color; ?> font-gotham-medium">
							Resources
						</button>
					</nav>
				</div>
					...
					if (have_rows('group_presenters')) :
						if ($presenter_count > 2) {
							...
							while (have_rows('group_presenters')) : the_row();
								...
								echo '<a href="' . get_permalink($presenter->ID) . '" class="text-left text-sm font-gotham text-slate-500 '. $consultant_hover_text_color .' transition">' . get_the_title($presenter->ID) . '</a>';
							endwhile;
						}
					endif;
					echo '</div>
		...
		<div x-show="section3 === true">
			...
			if ($resource_query->have_posts()) :
				...
				while ($resource_query->have_posts()) : $resource_query->the_post();
					...
					$types = get_the_terms(get_the_id(), 'resource_types');
					...
					echo '<div class="flex gap-4 group min-h-[120px] rounded-xl overflow-hidden bg-white shadow transition hover:shadow-lg">
							<a href="' . get_the_permalink() . '" class="w-40 h-auto">
								<div class="relative h-full">
									<div class="w-full absolute bg-gradient-to-bl from-black h-full transition duration-300 group-hover:opacity-0"></div>
									'.$resource_topic_bubble.'
									<div class="bg-center bg-cover h-full" style="background-image:url('.get_the_post_thumbnail_url('', 'medium').');"></div>
								</div>
							</a>
							<div class="flex-1 flex flex-col justify-center gap-2 py-4 pr-4">
								<a href="' . get_the_permalink() . '" class="block font-gotham-medium text-black transition '. $consultant_hover_text_color .'">
									' . $resource_title . '
								</a>
								<div class="flex gap-4 flex-wrap items-end">';
									if (!empty($tags)) :
        							    echo '<div class="flex flex-wrap items-center gap-1">
        							        <span class="font-gotham-bold text-sm mr-px uppercase ' . $video_color_fill . '">Tags:</span>';
        							        $tag_content = '';
        							        foreach ($tags as $term) {
        							            $tag_content .= '<a href="https://project-teach.launchpaddev.com/pt_resources/?keyword=&format%5B%5D=' . $term->slug . '" class="text-sm text-black font-gotham transtion duration-300 ' . $consultant_hover_text_color . '">' . $term->name . '</a>, ';
        							        }
									...
									if ($featured_posts) :
									echo '<div class="flex text-slate-500 gap-2">
						...
						if ($author_count > 2) {
							...
							foreach ($featured_posts as $post) :
								setup_postdata($post);
								echo '<a href="'.get_the_permalink().'" class="text-left text-sm font-gotham text-slate-500 '. $consultant_hover_text_color .'">' . get_the_title() . '</a>';
							endforeach;
							wp_reset_postdata();
						}
					endif;
