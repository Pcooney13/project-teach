...
<section class="section section__dd links__<?php echo $color;?>">
    <?php 
        // Title
        if ($pb_title) :
            echo '<h2 class="title title__dd text-' . $color . '">' . $pb_title . '</h2>';
        endif;
        // Text
        if ($pb_text) :
    ...
    ?>
        ...
        <div class="block border-<?php echo $color;?>">
            <?php if (get_sub_field('manually_enter')) : ?>       
                <?php if ( have_rows('writein') ) : ?>
                
                    <?php while( have_rows('writein') ) : the_row(); ?>
                        
                        <div class="flex-row flex items-center above-dd-row">
                            <!-- Image -->
                            <?php $image = get_sub_field('image');
                            if (!empty($image)): ?>
                                <a href="https://project-teach.launchpaddev.com/the-mental-health-effects-of-racism/" style="background-image:url(<?php echo esc_url($image['url']); ?>); background-size: cover; background-position: center;" class="flex-graphic" alt="COVID-19: Special Topics | Schooling in the Time of COVID, Part Three"></a>
                            <?php endif; ?>
                            <!-- Content -->
                            <div class=" flex-type">
                                <!-- Compact Title -->
                                <h3 class="text-<?php echo $color;?> mb-4">
                                ...
                                if ($link):
                                    ...
                                    ?>
                                    <a class="btn btn-<?php echo $color; ?>" href="<?php echo esc_url($link_url); ?>" target="<?php echo esc_attr($link_target); ?>"><?php echo esc_html($link_title); ?></a>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endwhile; ?>
                        ...
                        if ($count <= $pb_above_dd) : ?>
                                ...
                                <div <?php if ($count > 1) {echo 'class="mt-8"';} ?>>
                                    <div class="post__category post__category--list">
                                        <?php foreach($categories as $cat) : 
                                            ...
                                            ?>                                            
                                            <a style="color:<?php echo $cat_color; ?>;" href="/category/<?php echo $cat->slug;?>">
                                                <?php echo $cat->name; ?>
                                            </a>
                                        <?php endforeach; ?>
                                    </div>
                                    <h3 style="text-transform:none; font-weight:900; margin-top:0; margin-bottom:0; color:#000;" class="banner-title text-xl">
                                        <?php echo $title; ?>
                                    </h3>
                                </div>
                            <?php endif; ?>
                            <div class="flex-row flex items-center above-dd-row">
                                ...
                                <?php if (!empty(get_field('video'))) : ?>
                                    <div style="min-height:unset;" class="flex-graphic">
                                ...
                                <div class=" flex-type">
                                    ...
                                    <?php if ( have_rows('additional_resources', $featured_posts->ID) ) :
                                        while( have_rows('additional_resources', $featured_posts->ID) ) : the_row(); 
                                            if (get_sub_field('file_or_link') === 'link') :
                                                $link = get_sub_field('link');
                                                if ($link):
                                                    $link_url = $link['url'];
                                                    $link_title = $link['title'];
                                                    $link_target = $link['target'] ? $link['target'] : '_self';
                                                    ?>
                                                    <a class="text-blue flex text-base mb-4" href="<?php echo esc_url($link_url); ?>" target="<?php echo esc_attr($link_target); ?>"><?php echo esc_html($link_title); ?></a>
                                                <?php endif; 
                                            else:
                                    ...
                                    <?php if (get_sub_field('show_read_more_button')) : ?>
                                        <a id="join-consult" href="<?php echo $page_link; ?>" title="Read More" class="btn btn-<?php echo $color;?>">
                            ...
                            <div class="col-md-12">
                                <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
                                    <div class="panel panel-default">
                                        <div class="panel-heading bg-<?php echo $color;?>">
